...
public class Quat {
	...
	public Mat4 toRotationMatrix() {
		...
		this.renorm();
		res.mat[0][0] = 1 - 2 * (float) (j * j + k * k);
		res.mat[1][0] = 2 * (float) (i * j - r * k);
		res.mat[2][0] = 2 * (float) (r * j + i * k);

		res.mat[0][1] = 2 * (float) (i * j + r * k);
		res.mat[1][1] = 1 - 2 * (float) (i * i + k * k);
		res.mat[2][1] = 2 * (float) (k * j - i * r);

		res.mat[0][2] = 2 * (float) (i * k - r * j);
		res.mat[1][2] = 2 * (float) (k * j + i * r);
		res.mat[2][2] = 1 - 2 * (float) (i * i + j * j);
		return res;
	}
	...
	public static Quat fromEulerAngle(double angle, double x, double y, double z) {
		return new Quat(Math.cos(angle / 2), Math.sin(angle / 2) * x, Math
				.sin(angle / 2)
				* y, Math.sin(angle / 2) * z);
	}
	...
	public Point3D toEulerAngle() {
		...
		if (s < 0.0001f)
			return new Point3D((float) angle, (float) (1), (float) (0),
					(float) (0));
		return new Point3D((float) angle, (float) (x / s), (float) (y / s),
				(float) (z / s));
	}
